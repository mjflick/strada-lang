...
func main() int {
    ...
    my scalar $producer = thread::create(func () {
        for (my int $i = 1; $i <= 5; $i = $i + 1) {
            sys::usleep(100000);  # Simulate work
...
func main() int {
    ...
    # Read response
    my str $response = sys::socket_recv($sock, 4096);
    say("Response:");
    say($response);
...
func main() int {
    ...
    if ($libm == 0) {
        die("Failed to load libm: " . sys::dl_error());
    }
    ...
    # Call sqrt(16.0)
    my scalar $args = [16.0];
    my num $result = sys::dl_call_num($sqrt_fn, $args);
    say("sqrt(16) = " . $result);

    # Get sin function
    my int $sin_fn = sys::dl_sym($libm, "sin");
    $args = [3.14159 / 2];
    $result = sys::dl_call_num($sin_fn, $args);
    say("sin(pi/2) = " . $result);
...
func main() int {
    ...
    # Call my_add
    my int $add_fn = sys::dl_sym($lib, "my_add");
    my int $sum = sys::dl_call_int_sv($add_fn, [10, 20]);
    say("my_add(10, 20) = " . $sum);

    # Call my_greet
    my int $greet_fn = sys::dl_sym($lib, "my_greet");
    my str $greeting = sys::dl_call_str_sv($greet_fn, ["World"]);
    say($greeting);
