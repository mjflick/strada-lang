...
```strada
# Read entire file
my str $content = sys::slurp("input.txt");
say($content);
...
sys::write_fd($fh, "Line 1\n");
sys::write_fd($fh, "Line 2\n");
sys::close($fh);

# Append to file
my scalar $fh = sys::open("log.txt", "a");
sys::write_fd($fh, "New log entry\n");
sys::close($fh);
```
