...
- **--shared**
  Compile as a shared library (.so). The library can be loaded at runtime with `import_lib` or via `sys::dl_open()`.
...
stradadoc sys              # sys:: namespace functions
...
**stradac**(1), **strada-repl**(1), **stradadoc**(1), **gcc**(1), **gdb**(1)
