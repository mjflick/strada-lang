...
- `sys::fork()` in Strada → `strada_fork()` in C
- `say()` in Strada → `strada_say()` in C (core functions have no namespace)

The namespace prefix (`sys::`, `math::`) is purely a compile-time organization. The runtime C functions always use the `strada_` prefix.
