...
static void gen_statement(CodeGen *cg, ASTNode *stmt) {
    switch (stmt->type) {
        ...
        case NODE_IF_STMT: {
            ...
            gen_expression(cg, stmt->data.if_stmt.condition);
            emit(cg, ")) ");
            gen_block(cg, stmt->data.if_stmt.then_block);
            
            for (int i = 0; i < stmt->data.if_stmt.elsif_count; i++) {
                emit(cg, " else if (strada_to_bool(");
                gen_expression(cg, stmt->data.if_stmt.elsif_conditions[i]);
                emit(cg, ")) ");
        ...
        case NODE_WHILE_STMT:
            emit_indent(cg);
            emit(cg, "while (strada_to_bool(");
            gen_expression(cg, stmt->data.while_stmt.condition);
            emit(cg, ")) ");
        ...
        case NODE_FOR_STMT:
            ...
            if (stmt->data.for_stmt.condition) {
                ...
                gen_expression(cg, stmt->data.for_stmt.condition);
                emit(cg, ")");
            }
            emit(cg, "; ");
        ...
        case NODE_RETURN_STMT:
            ...
            if (stmt->data.return_stmt.value) {
                emit(cg, " ");
                // Special handling: main function or extern function returns raw value
                // Non-extern, non-main functions need wrapped values
                if (cg->current_func_is_main || cg->current_func_is_extern) {
                    ...
                    // Non-extern function returns StradaValue*
                    gen_expression(cg, stmt->data.return_stmt.value);
                }
            }
            emit(cg, ";\n");
    ...
    switch (expr->type) {
        ...
        case NODE_BINARY_OP: {
            ...
            if (cg->current_func_is_extern) {
                ...
                emit(cg, "strada_new_int(strada_to_num(");
                gen_expression(cg, expr->data.binary_op.left);
                emit(cg, ") == strada_to_num(");
                gen_expression(cg, expr->data.binary_op.right);
                emit(cg, "))");
        ...
        case NODE_UNARY_OP: {
            if (strcmp(expr->data.unary_op.op, "-") == 0) {
                emit(cg, "strada_new_num(-strada_to_num(");
                gen_expression(cg, expr->data.unary_op.operand);
                emit(cg, "))");
        ...
        case NODE_ASSIGN: {
            ...
            if (expr->data.assign.target->type == NODE_MEMBER_ACCESS) {
                ...
                if (strcmp(expr->data.assign.op, "=") == 0) {
                    ...
                    switch (field_type) {
                        case TYPE_STR:
                            // For string fields, use strcpy
                            emit(cg, "strcpy(");
                            gen_expression(cg, ma->data.member_access.object);
                            emit(cg, "->%s, strada_to_str(", ma->data.member_access.field_name);
                ...
                emit(cg, "->value.hv, strada_to_str(");
        ...
        case NODE_CALL: {
            ...
            if (strcmp(expr->data.call.name, "say") == 0) {
                ...
                emit(cg, "strada_warn(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ")");
                for (int i = 1; i < expr->data.call.arg_count; i++) {
                    emit(cg, ", strada_to_str(");
                ...
                emit(cg, "strada_new_int(strada_length(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ")))");
                ...
                emit(cg, "strada_die(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, "))");
                ...
                if (expr->data.call.args[0]->type == NODE_HASH_ACCESS) {
                    ...
                    emit(cg, "strada_new_int(strada_hash_exists(");
                    gen_expression(cg, ha->data.subscript.array);
                    emit(cg, "->value.hv, strada_to_str(");
                    gen_expression(cg, ha->data.subscript.index);
                    emit(cg, ")))");
                ...
                emit(cg, "strada_new_str(strada_upper(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ")))");
            } else if (strcmp(expr->data.call.name, "lower") == 0 || strcmp(expr->data.call.name, "lc") == 0) {
                emit(cg, "strada_new_str(strada_lower(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ")))");
                ...
                emit(cg, "strada_new_str(strada_reverse(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ")))");
                ...
                emit(cg, "strada_new_str(strada_chr(strada_to_int(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ")))");
                ...
                emit(cg, "strada_new_int(strada_rindex(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, "), strada_to_str(");
                gen_expression(cg, expr->data.call.args[1]);
                emit(cg, ")))");
                ...
                emit(cg, "(({ StradaValue *__sv = strada_new_array(); ");
                emit(cg, "__sv->value.av = strada_regex_capture(strada_to_str(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, "), strada_to_str(");
                gen_expression(cg, expr->data.call.args[1]);
                emit(cg, ")); __sv; }))");
                ...
                emit(cg, "strada_new_int(strada_socket_send(");
                gen_expression(cg, expr->data.call.args[0]);
                emit(cg, ", strada_to_str(");
                gen_expression(cg, expr->data.call.args[1]);
                emit(cg, ")))");
